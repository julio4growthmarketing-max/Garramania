...
public class ClawController : MonoBehaviour
{
    ...
    {
        // Rigidbody cinemático parentado a um transform móvel pode preservar uma
        // pose física antiga em alguns frames. Reaplicamos a pose do wrapper no
        // socket mecânico depois que a garra terminou de se mover neste frame.
        if (premioAgarrado == null || carrySocket == null) return;
        Prize prize = premioAgarrado.GetComponent<Prize>();
        if (prize == null || prize.State != PrizeState.Attached) return;
        premioAgarrado.transform.SetPositionAndRotation(carrySocket.position, carrySocket.rotation);
    }
    ...
    private Prize FindBestCaptureCandidate()
    {
        ...
        Vector3 capturePoint = carrySocket != null ? carrySocket.position : transform.position;
        float captureY = capturePoint.y;
    ...
    void ConstruirGarra()
    {
        ...
        carrySocketObj.transform.localPosition = new Vector3(0f, -0.82f, 0f);
