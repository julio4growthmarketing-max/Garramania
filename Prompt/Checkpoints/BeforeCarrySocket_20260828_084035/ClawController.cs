...
public class ClawController : MonoBehaviour
{
    ...
    private System.Collections.IEnumerator RotinaCicloFliperama()
    {
        ...
        // 5. FASE DE ENTREGA E VITÓRIA
        bool haviaPremio = premioAgarrado != null;
        Prize premioEntregue = haviaPremio ? premioAgarrado.GetComponentInParent<Prize>() : null;
        
        AbrirGarraFisica();

        // Se capturou prêmio e levou até a calha, celebração imediata!
        if (haviaPremio && premioEntregue != null)
        {
            premioEntregue.MarkDelivered();
            if (GameSession.Instance != null)
            {
                GameSession.Instance.RegisterPrizeDelivered(premioEntregue);
            }
            Debug.Log($"[ClawController] VITÓRIA! Prêmio entregue na calha: {premioEntregue.prizeId}");
        }
    ...
    private void FecharGarraFisica()
    {
        ...
        if (candidate != null)
        {
            ...
            candidate.Attach(clawVisualContainer != null ? clawVisualContainer : transform);
            grabSuccess = true;
    ...
    private Prize FindBestCaptureCandidate()
    {
        ...
        Prize best = null;
        float bestHorizDist = 0.65f; // Raio confortável que cobre a largura dos dentes abertos
