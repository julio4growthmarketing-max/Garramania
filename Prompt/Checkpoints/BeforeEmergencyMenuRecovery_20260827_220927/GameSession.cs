...
public class GameSession : MonoBehaviour
{
    ...
    [Tooltip("Fallback temporário: inicia a partida após um frame para validar gameplay mesmo se a UI não receber clique. Desative quando o menu estiver validado.")]
    [SerializeField] private bool prototypeAutoStart = true;
    ...
    void Start()
    {
        ...
        OnCreditsChanged?.Invoke(Credits);
        OnHighScoreChanged?.Invoke(HighScore);
        if (prototypeAutoStart) StartCoroutine(BootstrapPrototypeAfterFrame());
    }

    private System.Collections.IEnumerator BootstrapPrototypeAfterFrame()
    {
        yield return null;
        if (CurrentState == GameState.Idle) StartGame();
    }
